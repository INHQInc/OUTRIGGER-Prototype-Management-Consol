/**
 * THE FULL OBSERVATION — one measure, read properly.
 *
 * The line in the observations list is a teaser. This is the thing you can
 * read back in a meeting: what the variation did to this measure, why it
 ...
 * verdict is computed and belongs to the decision measure alone.
 */
...
const tool = {
  ...
  description: "A full read of ONE measure in this experiment, for the hotel's team.",
  input_schema: {
    type: "object" as const,
    properties: {
      headline: { type: "string" as const, description: "<=90 chars, NO DIGITS. What this measure is telling the business, in one line." },
      read: { type: "string" as const, description: "<=600 chars, NO DIGITS. What guests did differently on this surface between the two versions, and WHY that follows from what was built. Name the surfaces and the actions in the reader's words." },
      context: { type: "string" as const, description: "<=400 chars, NO DIGITS. How this bears on what the experiment set out to prove, and on the booking path either side of it — where intent is created, where it leaks." },
      caution: { type: "string" as const, description: "<=300 chars, NO DIGITS. What would make this reading wrong: a surface only one version has, clicks moving between places rather than new demand, a measure that counts actions rather than guests, too little data. Omit if there is genuinely nothing." },
      watch: { type: "string" as const, description: "<=200 chars, NO DIGITS. The one thing to watch next on this measure." },
    },
...
export async function deepObservation(opts: {
  ...
  const m = opts.stats?.metrics.find((x) => x.key === opts.measureKey);
  if (!m) throw new Error("That measure isn't reporting.");
  ...
  const comp = opts.map?.composites.find((c) => `composite:${c.id}` === opts.measureKey);
  const howBuilt = comp
  ...
  const neighbours = (opts.stats?.metrics ?? [])
    .filter((x) => x.key !== opts.measureKey)
    .map((x) => {
...
NO DIGITS in your words — every number is printed beside your sentences and would go stale the moment the counts move. No statistics vocabulary: no significance, no sample size, no confidence, no days remaining. Do not give a verdict on the experiment; that belongs to the decision measure and the console computes it.`,
  ...
  const read = clean(raw.read, 600);
  const context = clean(raw.context, 400);
  if (!headline || !read) throw new Error("The observation didn't meet the format — try again.");

  return {
    measureKey: opts.measureKey,
    headline, read, context,
    caution: clean(raw.caution, 300) || undefined,
    watch: clean(raw.watch, 200) || undefined,
