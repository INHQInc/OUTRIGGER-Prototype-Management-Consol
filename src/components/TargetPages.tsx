    ...
    <div className="space-y-4 max-w-2xl">
      ...
      <div className="rounded-xl border border-border bg-surface overflow-hidden">
        <div className="px-4 py-2.5 border-b border-border">
          <span className="text-[14px] font-semibold">Injection script</span>
          <span className="text-[13px] text-muted-2 ml-2">The dev adds this tag once to the site&apos;s global template (e.g. before <span className="font-mono">&lt;/head&gt;</span>). Inert until a page is opened with <span className="font-mono">?opmc</span>.</span>
        </div>
        {environments.length === 0 ? (
          <div className="px-4 py-3 text-[14px] text-muted-2">
          ...
          environments.map((env) => (
            <div key={env.id} className="px-4 py-3 border-b border-border/60 last:border-0 flex items-center justify-between gap-3">
              <pre className="text-[13px] font-mono text-muted-2 overflow-x-auto flex-1 min-w-0">{tagFor(env)}</pre>
              <button onClick={() => copyTag(env)} className="text-[14px] text-accent hover:text-accent-hover font-medium shrink-0">{copiedTag === env.id ? "Copied" : "Copy"}</button>
            </div>
          ))
        )}
      </div>
      ...
      <div className="rounded-xl border border-border bg-surface overflow-hidden">
        <div className="px-4 py-2.5 border-b border-border">
          <span className="text-[14px] font-semibold">Test pages</span>
          <span className="text-[13px] text-muted-2 ml-2">Where the prototype injects. Each page: <b>Open ↗</b> to preview with the review token, and <b>Verify</b> that the loader is deployed on it.</span>
        </div>
        <div className="p-4 space-y-2">
          <div className="flex items-center gap-2">
            ...
            <button onClick={add} disabled={busy || !adding.trim()} className="h-9 px-3 rounded-lg bg-accent text-accent-fg text-[15px] font-semibold hover:bg-accent-hover disabled:opacity-40 shrink-0">Add</button>
          </div>
          {err && <div className="text-[14px] text-danger">{err}</div>}
          {targets.length === 0 ? (
            <div className="text-[14px] text-muted-2 py-1">No pages yet — add the URL(s) this prototype changes.</div>
          ) : (
            <>
              <div className={`text-[13px] ${verifiedCount === targets.length ? "text-ok" : "text-muted-2"}`}>
    ...
    <div className="rounded-lg border border-border bg-surface-2/20 px-3 py-2.5">
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          <div className="font-mono text-[14px] text-foreground truncate">{url}</div>
          <InjectionBadge injection={inj} checking={checking} />
          {err && <div className="text-[13px] text-danger">{err}</div>}
        </div>
        <div className="flex items-center gap-3 shrink-0">
          <button onClick={() => act(false)} disabled={checking} className="text-[14px] px-2 py-0.5 rounded-md border border-border text-muted hover:text-foreground hover:border-border-strong disabled:opacity-40">{checking ? "Verifying…" : "Verify"}</button>
          {inj && !passing && <button onClick={() => act(true)} disabled={checking} title="I opened the page and saw the prototype inject" className="text-[14px] px-2 py-0.5 rounded-md border border-warn/50 text-warn hover:border-warn disabled:opacity-40">Confirm</button>}
          <button onClick={onCopyLink} className="text-[14px] text-muted-2 hover:text-foreground">{copied ? "Copied" : "Copy link"}</button>
          <a href={reviewLink} target="_blank" rel="noreferrer" className="text-[14px] text-accent hover:text-accent-hover font-medium">Open ↗</a>
          <button onClick={onRemove} disabled={busy} className="text-[14px] text-danger hover:opacity-80 disabled:opacity-40">Remove</button>
        </div>
      </div>
    </div>
  );
...
function InjectionBadge({ injection, checking }: { injection?: TargetInjection; checking: boolean }) {
  if (checking && !injection) return <div className="text-[13px] text-muted-2">Verifying injection…</div>;
  if (!injection) return <div className="text-[13px] text-muted-2">● Not verified yet — click Verify</div>;
  const who = injection.by ? ` · ${injection.by}` : "";
  switch (injection.state) {
    case "present": return <div className="text-[13px] text-ok">● Loader detected on this page{who}</div>;
    case "confirmed": return <div className="text-[13px] text-ok">● Confirmed injecting (human-verified){who}</div>;
    case "wrong-env": return <div className="text-[13px] text-warn">● Loader present but for {injection.foundEnvLabel ?? "another environment"} — Confirm if it still injects{who}</div>;
    case "absent": return <div className="text-[13px] text-danger">● Loader not found on this page — install the tag, or Confirm if it&apos;s injected client-side{who}</div>;
    default: return <div className="text-[13px] text-muted-2">● Couldn&apos;t reach the page to auto-check — open it and Confirm{who}</div>;
  }
}
