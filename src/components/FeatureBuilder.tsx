    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-border bg-surface overflow-hidden">
        <div className="h-11 border-b border-border flex items-center gap-3 px-3">
          <div className="flex items-center rounded-lg bg-background border border-border p-0.5">
            <button onClick={() => setVariant(false)} disabled={picking} className={`px-3 py-1 rounded-md text-[11px] font-medium ${!variant && !picking ? "bg-surface-2 text-foreground" : "text-muted-2 hover:text-foreground"}`}>Control</button>
            <button onClick={() => setVariant(true)} disabled={picking} className={`px-3 py-1 rounded-md text-[11px] font-medium ${variant && !picking ? "bg-accent text-accent-fg" : "text-muted-2 hover:text-foreground"}`}>Variant</button>
          </div>
          <div className="w-px h-5 bg-border" />
          <div className="flex items-center gap-1">
            {DEVICES.map((d) => (
              <button key={d.key} onClick={() => setDevice(d.key)} className={`px-2.5 py-1 rounded-md text-[11px] font-medium ${device === d.key ? "bg-surface-2 text-foreground" : "text-muted-2 hover:text-foreground"}`}>{d.key}</button>
            ))}
          </div>
          {picking && <span className="text-[11px] text-accent font-medium ml-2">Picking… click an element (Esc cancels)</span>}
      ...
      <div className="rounded-xl border border-border bg-surface">
        <div className="px-4 py-3 border-b border-border flex items-center justify-between">
          <h3 className="text-[13px] font-semibold">Injection points <span className="text-muted-2 font-normal">({rows.length})</span></h3>
          <div className="flex items-center gap-2">
            {msg && <span className="text-[11px] text-muted-2">{msg}</span>}
            <button onClick={save} disabled={!dirty || saving} className="h-8 px-3 rounded-lg bg-accent text-accent-fg text-[12px] font-semibold hover:bg-accent-hover disabled:opacity-40">
              {saving ? "Saving…" : "Save"}
            </button>
          </div>
        </div>
        ...
        <div className="divide-y divide-border">
          {rows.map((r, i) => {
            ...
            return (
              <div key={i} className="px-4 py-3 space-y-2">
                <div className="flex items-center gap-2">
                  <span className={`inline-flex px-2 py-0.5 rounded-full text-[11px] font-medium border ${r.type === "html" ? "border-accent/40 text-accent" : "border-border text-muted"}`}>{r.type}</span>
                  {r.type === "html" ? (
                    <input value={r.fragment ?? ""} onChange={(e) => update(i, { fragment: e.target.value })} placeholder="fragment.html" className="w-44 h-7 rounded-md bg-background border border-border px-2 text-[11px] font-mono focus:border-accent focus:outline-none" />
                  ) : (
                    <input value={r.file ?? ""} onChange={(e) => update(i, { file: e.target.value })} placeholder={r.type === "css" ? "overlay.css" : "overlay.js"} className="w-44 h-7 rounded-md bg-background border border-border px-2 text-[11px] font-mono focus:border-accent focus:outline-none" />
                  )}
                  <button onClick={() => remove(i)} className="ml-auto text-[11px] text-danger hover:opacity-80">Remove</button>
                </div>

                {r.type === "html" && (
                  <div className="flex items-center gap-2">
                    <select value={r.placement} onChange={(e) => update(i, { placement: e.target.value })} className="h-7 rounded-md bg-background border border-border px-2 text-[11px] focus:border-accent focus:outline-none">
                      {PLACEMENTS.map((p) => <option key={p.value} value={p.value}>{p.label}</option>)}
                    </select>
                    {needsAnchor && (
                      <>
                        <input value={r.selector} onChange={(e) => update(i, { selector: e.target.value })} placeholder=".class or #id or path" className="flex-1 h-7 rounded-md bg-background border border-border px-2 text-[11px] font-mono focus:border-accent focus:outline-none" />
                        <button onClick={() => { setPickingIdx(i); setNonce((n) => n + 1); }} className="h-7 px-3 rounded-md bg-surface-2 border border-border text-[11px] font-medium hover:border-accent hover:text-accent whitespace-nowrap">
                          ⌖ Pick
                        </button>
                      </>
                    )}
          ...
          {rows.length === 0 && <div className="px-4 py-6 text-center text-[12px] text-muted-2">No injections yet.</div>}
        </div>

        <div className="px-4 py-3 border-t border-border flex gap-2">
          <button onClick={() => add("html")} className="h-8 px-3 rounded-lg border border-border text-[12px] font-medium hover:border-accent hover:text-accent">+ HTML</button>
          <button onClick={() => add("css")} className="h-8 px-3 rounded-lg border border-border text-[12px] font-medium hover:border-accent hover:text-accent">+ CSS</button>
          <button onClick={() => add("js")} className="h-8 px-3 rounded-lg border border-border text-[12px] font-medium hover:border-accent hover:text-accent">+ JS</button>
        </div>
      </div>
    </div>
  );
