...
export default async function Dashboard() {
  ...
  protos.forEach((p, i) => {
    ...
    if (v?.certification && !v.certification.passed) {
      attention.push({ text: `Certification failed on ${p.name} v${v.version} — the push is gated until it's fixed and re-cut.`, href: `/prototypes/${p.key}?tab=experiment`, action: "Open" });
    }
    if (push && v && push.version < v.version) {
      attention.push({ text: `${p.name}: Optimizely is running v${push.version}, latest cut is v${v.version}.`, href: `/prototypes/${p.key}?tab=experiment`, action: "Push update" });
    }
    if (push && push.verified === false) {
      attention.push({ text: `${p.name}: last push did not read-back verify — inspect the variation before publishing.`, href: `/prototypes/${p.key}?tab=experiment`, action: "Open" });
    }
  });
