...
- **Workspace = rooms, not steps** (07-24, twice-reworked to the user's own tab spec): the six-dot stepper is DELETED. Header = stage chip (`pipeline.stage` — ONE derivation shared with the board column; board.ts's own columnFromPipeline removed) + that stage's status line + the single CTA + ground-truth/alerts. Rooms (`?tab=`): **Overview | Brief | Source Control | Skills | Build | Review | Experimentation | Handoff** (`app/prototypes/[key]/page.tsx`; legacy build→source, pages→review aliases). Source Control = SETUP only (Repo & branch + Local folders → localStorage). Build = the develop loop (Prepare the branch → Start Claude → Sync brief & skills); carries the build-step dot. Skills = inherited (global/brand) vs this-prototype sections. Review = TargetPages on the real env. **Experimentation** = cut (SourcePanel) + ShipPanel with project picker + bind-existing OR **create-from-prototype** (POST `/api/optimizely/experiments` `create:{name}` → paused draft, URL-targeted from target[0], 50/50, seeded with latest cut UNLESS cert failed → placeholder; auto-binds; admin-only `setProjectId`). **Handoff** = `HandoffExplorer` (VS-style read-only tree+code at the CUT version's exact SHA via `/api/prototypes/handoff-files`, traversal-guarded; memoized CodePane, 150K tokenize cap) + HandoffPanel record. `PrototypeOverview` = needs-attention (deduped vs header alerts) + brief-as-document + parts grid + activity feed from audit events/cuts/pushes/Claude check-ins.
...
- **Draft-with-AI brief composer**: `/api/prototypes/brief-draft` → `lib/ai/brief.ts` (@anthropic-ai/sdk, claude-opus-4-8, forced-tool JSON). System prompt = the `opmc-brief-author` skill (`delivery: "console"`). Requires `ANTHROPIC_API_KEY` (Vercel). Now a full loop:
  ...
  - **References** — `brief.references[]` (BriefReference, `referenceKind()` auto-types figma/image/design/doc/link); live in the Draft-with-AI drawer as AI inputs, passed to draft + refine (even unsaved), written into `.opmc/brief.md` for the building agent. Links only — image upload needs a blob store (not wired).
- **Four-color status vocabulary** (`lib/prototypes/severity.ts`, client-safe, TYPE-only imports): 🔴 critical (blocked) · 🟠 attention (current step / warn on a done one) · 🟢 good · ⚪ pending. `stepSeverity()` derives it once; tab dots, board cards, list rows all color from it. Overview has a **vertical checklist** (`pipeline.checklist`) — one row per pipeline-backed room, colored + description + link, most-urgent first; it IS the Overview status home (header status strip hidden there; header CTA hidden when you're already in its target room).
