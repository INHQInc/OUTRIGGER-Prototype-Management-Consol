...
- **\`.opmc/customer.md\`** — **whose site this is, and the words they use for it.** Read it BEFORE you write a single string that {{visitorNounPlural}} will see: copy, labels, alt text, empty and error states. It is resolved from the console's brand profile for this customer, not a default — so if a word in it looks wrong, the profile is wrong and fixing it there fixes it everywhere. \`context.json\` → \`customer\` carries the same facts as data.
