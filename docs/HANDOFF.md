...
- **Draft-with-AI brief composer**: `/api/prototypes/brief-draft` → `lib/ai/brief.ts` (@anthropic-ai/sdk, claude-opus-4-8, forced-tool JSON). System prompt = the `opmc-brief-author` skill (`delivery: "console"`). Requires `ANTHROPIC_API_KEY` (Vercel). Now a full loop:
  - **Terminates** — default ZERO questions; ask only when an answer changes what's built; ≤2 and first-pass only; the answers pass is TERMINAL (code force-empties questions). No more endless follow-ups.
...
1. **S4 results read-back** — experiment status/results on table rows + workspace (also enables live-brief vs running-version briefSnapshot drift display).
