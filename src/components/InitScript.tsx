    ...
    return (
      <div className="rounded-xl border border-border bg-surface overflow-hidden">
        <div className="px-4 py-2.5 border-b border-border">
          ...
          <span className="text-[13px] text-muted-2 ml-2">Writes the brief, page snapshots and selected skills into the branch — Claude wakes up loaded.</span>
        </div>
    ...
    <div className="space-y-3">
      {path ? (
        <div className="rounded-xl border border-accent/40 bg-[color-mix(in_srgb,var(--accent)_4%,transparent)] overflow-hidden">
          ...
          <div className="px-4 pb-3 text-[13px] text-muted-2 border-t border-border/60 pt-2.5">
            ...
            {src
              ...
              : <> Add the website source checkout in <Link href={`/prototypes/${prototypeKey}?tab=source`} className="text-accent hover:text-accent-hover">Source Control</Link> and Claude can build against real components instead of only the page snapshot.</>}
        ...
        <div className="rounded-xl border border-warn/40 bg-[color-mix(in_srgb,var(--warn)_6%,transparent)] px-4 py-3 text-[14px] text-warn">
          No local folder saved — <Link href={`/prototypes/${prototypeKey}?tab=source`} className="text-accent hover:text-accent-hover font-medium">set it in Source Control →</Link> to generate your Start Claude command.
        </div>
      )}
