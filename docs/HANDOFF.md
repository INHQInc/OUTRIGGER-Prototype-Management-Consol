...
- `ba6337c` (fail-closed tenant guards) IS pushed + deployed READY (verified, zero runtime errors). Still LOCAL (user runs `git push`): `1e1272b` deleteOrg cascade, `518de0d` heartbeat id length, `de0bc0c` dead-links/counts, `34a15cc` docs, `9477aae`… wait — see `git log origin/main..HEAD` for the live list. Then the **prototype setup-checklist rework** commit + the 2nd-pass fix bundle. Verify deploy after via Vercel MCP (project `prj_k2NQb2qYTAN2rlgHwW7D4KLOIONx`, team `team_VYlQ8CLTxGgRpafO8hbu5Gmz`).
