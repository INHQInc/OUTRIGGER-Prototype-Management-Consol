  ...
  return (
    <div className="rounded-xl border border-border bg-surface overflow-hidden">
      <div className="px-4 py-2.5 border-b border-border">
        <span className="text-[14px] font-semibold">Experiment</span>
        <span className="text-[13px] text-muted-2 ml-2">Creates a paused Optimizely draft with the frozen version — a human starts traffic.</span>
      </div>
      ...
      {error && <div className="px-4 pb-2 text-[14px] text-danger">{error}</div>}

      <details className="border-t border-border">
        <summary className="px-4 py-2 text-[13px] text-muted-2 cursor-pointer hover:text-foreground">All environments & history</summary>
        <div className="border-t border-border">
              ...
              <div key={env.id} className="px-4 py-3 border-b border-border last:border-0">
                <div className="flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2 min-w-0">
                    <span className="text-[15px] font-medium">{env.label}</span>
                    <Badge tone={env.kind === "production" ? "accent" : "neutral"}>{env.kind}</Badge>
                    <span className="text-[12.5px] text-muted-2 uppercase tracking-wide">{vehicle}</span>
                  </div>
                  {canPromote && (
                    <div className="flex items-center gap-2 shrink-0">
                      <select
                        ...
                        className="rounded-lg bg-background border border-border px-2 py-1.5 text-[14px] text-foreground focus:border-accent focus:outline-none"
                      >
                        {versions.map((v) => <option key={v.id} value={v.id}>v{v.version} · {v.gitSha.slice(0, 7)}</option>)}
                      </select>
                      <button onClick={() => promote(env.id)} disabled={!versions.length || busyEnv === env.id} className="h-8 px-3 rounded-lg bg-accent text-accent-fg text-[14px] font-semibold hover:bg-accent-hover disabled:opacity-40 transition-colors">
          ...
          {promotions.length > 0 && (
            <div className="border-t border-border">
              <div className="px-4 py-2 text-[12.5px] font-semibold uppercase tracking-wider text-muted-2">History</div>
              {promotions.slice(0, 8).map((p) => (
                <div key={p.id} className="flex items-center justify-between gap-3 px-4 py-2 text-[13px] border-t border-border">
