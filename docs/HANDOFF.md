...
- **Workspace = rooms, not steps** (07-24, twice-reworked to the user's own tab spec): the six-dot stepper is DELETED. Header = stage chip (`pipeline.stage` — ONE derivation shared with the board column; board.ts's own columnFromPipeline removed) + that stage's status line + the single CTA + ground-truth/alerts. Rooms (`?tab=`): **Overview | Brief | Source Control | Skills | Review | Experimentation | Handoff** (`app/prototypes/[key]/page.tsx`; legacy build→source, pages→review aliases). Source Control = repo/branch + local folders + "Start Claude" + "Sync brief & skills to the branch". Skills = inherited (global/brand) vs this-prototype sections. Review = TargetPages on the real env. **Experimentation** = cut (SourcePanel) + ShipPanel with project picker + bind-existing OR **create-from-prototype** (POST `/api/optimizely/experiments` `create:{name}` → paused draft, URL-targeted from target[0], 50/50, seeded with latest cut UNLESS cert failed → placeholder; auto-binds; admin-only `setProjectId`). **Handoff** = `HandoffExplorer` (VS-style read-only tree+code at the CUT version's exact SHA via `/api/prototypes/handoff-files`, traversal-guarded; memoized CodePane, 150K tokenize cap) + HandoffPanel record. `PrototypeOverview` = needs-attention (deduped vs header alerts) + brief-as-document + parts grid + activity feed from audit events/cuts/pushes/Claude check-ins.
