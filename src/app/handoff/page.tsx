  ...
  return (
    <>
      ...
      <div className="flex-1 overflow-y-auto px-8 py-6">
        {!repo && (
          <div className="mb-4 rounded-lg border border-danger/40 bg-[color-mix(in_srgb,var(--danger)_8%,transparent)] px-4 py-3 text-[14px] text-muted">
          ...
          <div className="grid grid-cols-2 gap-4">
            {features.map((f) => (
              <Link key={f.key} href={`/handoff/${f.key}`} className="rounded-xl border border-border bg-surface p-5 hover:border-border-strong transition-colors">
                ...
                <div className="text-[14px] text-muted-2 mt-0.5 font-mono">{f.key}</div>
                <p className="text-[14px] text-muted mt-3 leading-relaxed line-clamp-2">{f.description}</p>
                <div className="text-[14px] text-accent hover:text-accent-hover font-medium mt-3">Open compare →</div>
              </Link>
            ))}
          </div>
        )}
