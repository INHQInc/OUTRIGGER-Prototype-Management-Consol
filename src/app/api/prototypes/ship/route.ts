...
export async function POST(req: NextRequest) {
  let body: { key?: string; bind?: { experimentId?: string; variationId?: string; experimentName?: string; variationName?: string }; push?: boolean; version?: number; override?: boolean; coverageAck?: boolean };
  ...
  try {
    ...
    if (body.push) {
      const result = await pushToOptimizely(g.proto.key, { version: typeof body.version === "number" ? body.version : undefined, override: body.override, coverageAck: body.coverageAck, actor });
