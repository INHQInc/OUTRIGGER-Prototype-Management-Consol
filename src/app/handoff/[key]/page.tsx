      ...
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2">
            <Link href="/handoff" className="h-9 px-3 flex items-center rounded-lg text-[15px] text-muted hover:text-foreground">← Handoff</Link>
            <a href={`/preview/feature/${key}?variant=1`} target="_blank" rel="noreferrer" className="h-9 px-3 flex items-center rounded-lg text-[15px] text-muted hover:text-foreground border border-border">Prototype ↗</a>
            <a href={`/api/handoff/${key}/patch`} className="h-9 px-4 flex items-center rounded-lg bg-accent text-accent-fg text-[15px] font-semibold hover:bg-accent-hover">Download .patch</a>
          </div>
        }
      />
      <div className="flex-1 overflow-y-auto px-8 py-6 space-y-6">
        {err && <div className="text-[15px] text-danger">{err}</div>}
        {!pkg && !err && <div className="text-[15px] text-muted-2">Computing compare…</div>}

        {pkg && (
          <>
            <div className="rounded-xl border border-border bg-surface p-4">
              <p className="text-[15px] text-muted leading-relaxed">{pkg.summary}</p>
              {pkg.warnings.length > 0 && (
                <ul className="mt-3 space-y-1">
                  {pkg.warnings.map((w, i) => (
                    <li key={i} className="text-[13px] text-muted-2">• {w}</li>
                  ))}
                </ul>
              )}
            </div>
            ...
            {pkg.pieces.map((p, i) => (
              <div key={i} className="space-y-3">
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <span className="text-[14px] text-muted">{p.placement}</span>
                </div>
                ...
                {p.kind === "html" && p.candidates && p.candidates.length > 0 && (
                  <div className="flex items-center gap-2 text-[14px]">
                    ...
                    <select
                      ...
                      className="h-8 rounded-md bg-background border border-border px-2 text-[13px] font-mono focus:border-accent focus:outline-none max-w-[520px]"
                    >
                      {p.candidates.map((c) => (
                        <option key={c.file} value={c.file}>{c.block} — {c.file}</option>
                      ))}
                    </select>
                    {p.candidates.length > 1 && <span className="text-muted-2 text-[13px]">{p.candidates.length} candidates — pick the right one</span>}
                  </div>
                )}
                {p.note && <div className="text-[13px] text-muted-2">{p.note}</div>}
