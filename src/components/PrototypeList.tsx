  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-1.5 flex-wrap">
        {chips.map((ch) => (
          <button key={ch.id} onClick={() => setFilter(ch.id)}
            className={`px-2.5 py-1 rounded-full border text-[11px] font-medium transition-colors ${filter === ch.id ? "border-accent bg-[color-mix(in_srgb,var(--accent)_10%,transparent)] text-foreground" : `border-border text-muted hover:text-foreground ${ch.tone ?? ""}`}`}>
      ...
      <div className="rounded-xl border border-border bg-surface overflow-hidden">
        <div className="grid grid-cols-[1fr_6.2rem_7.2rem_1fr_8rem] gap-3 px-4 py-2 border-b border-border text-[10px] font-semibold uppercase tracking-wider text-muted-2">
          <span>Prototype</span><span>Stage</span><span>Pipeline</span><span>Next</span><span>Truth</span>
        </div>
        {shown.length === 0 ? (
          <div className="px-4 py-8 text-center text-[12px] text-muted-2">Nothing matches this filter.</div>
        ) : shown.map((c) => {
          ...
          return (
            <Link key={c.key} href={`/prototypes/${c.key}`} className="grid grid-cols-[1fr_6.2rem_7.2rem_1fr_8rem] gap-3 px-4 py-3 border-b border-border/60 last:border-0 items-center hover:bg-surface-2/30 transition-colors">
              <span className="min-w-0">
                <span className="text-[12.5px] font-semibold block truncate">{c.name}</span>
                {c.hypothesis && <span className="text-[10.5px] text-muted-2 block truncate">{c.hypothesis}</span>}
              </span>
              <span className={`text-[11px] font-medium ${c.column === "testing" ? "text-warn" : c.column === "shipped" ? "text-ok" : "text-muted"}`}>
                {c.locked ? "🔒 " : ""}{col.label}
              </span>
              ...
              <span className="min-w-0">
                <span className={`text-[11px] block truncate ${blocked.length ? "text-danger" : "text-muted"}`}>
                  {blocked.length ? `⚠ ${blocked.map((s) => `${s.title}: ${s.status}`).join(" · ")}` : c.pipeline.primaryAction.label}
                </span>
              </span>
              <span className="flex items-center gap-1 flex-wrap">
                {c.pipeline.truth.latestVersion != null && <span className="text-[9px] px-1.5 py-0.5 rounded bg-surface-2 text-muted font-mono">v{c.pipeline.truth.latestVersion}{c.pipeline.truth.certified === true ? "✓" : c.pipeline.truth.certified === false ? "✗" : ""}</span>}
                {c.experimentStatus && <span className={`text-[9px] px-1.5 py-0.5 rounded ${c.experimentStatus === "running" ? "text-warn bg-[color-mix(in_srgb,var(--warn)_12%,transparent)]" : "bg-surface-2 text-muted-2"}`}>{c.experimentStatus.replace("_", " ")}</span>}
              </span>
            </Link>
          );
        })}
