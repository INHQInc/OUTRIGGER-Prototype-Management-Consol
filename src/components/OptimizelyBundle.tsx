...
export function OptimizelyBundle({ prototypeKey, name, metric, targetUrls, version, variationJs }: {
  ...
  const header = (
    <div className="px-4 py-2.5 border-b border-border">
      <span className="text-[12px] font-semibold">Optimizely bundle</span>
      <span className="text-[11px] text-muted-2 ml-2">Everything to run this as a Web Experiment by hand.</span>
    </div>
  );

  if (!version) {
    return <div className="rounded-xl border border-border bg-surface overflow-hidden">{header}<div className="px-4 py-3 text-[12px] text-muted-2">Cut a version first — the bundle packages the frozen build.</div></div>;
  }
  if (!variationJs) {
    return <div className="rounded-xl border border-border bg-surface overflow-hidden">{header}<div className="px-4 py-3 text-[12px] text-warn">v{version} has no code snapshot — cut a fresh version from repo.</div></div>;
  }
    ...
    <div className="rounded-xl border border-accent/40 bg-[color-mix(in_srgb,var(--accent)_4%,transparent)] overflow-hidden">
      ...
      <div className="p-4 space-y-4 text-[12px]">
        <div>
          ...
          <p className="text-[11px] text-muted-2">Paste as the variation&apos;s <b>custom JavaScript</b> — it&apos;s self-contained (injects its own CSS/HTML/behavior), so nothing else to add.</p>
        </div>
          ...
          <div className="space-y-1">
            {targetUrls.length === 0 ? <span className="text-muted-2">No target pages set.</span> : targetUrls.map((u) => (
              <div key={u} className="flex items-center justify-between gap-3">
                <span className="font-mono text-[11px] text-muted truncate">{u}</span>
                <button onClick={() => copy(u, u)} className="text-muted-2 hover:text-foreground shrink-0">{copied === u ? "Copied" : "Copy"}</button>
              </div>
            ))}
          </div>
          <p className="text-[11px] text-muted-2 mt-1">Target the experiment to these page(s) (exact URL, or a path match).</p>
        </div>
        ...
        <ol className="list-decimal ml-4 space-y-1 text-[11px] text-muted-2 border-t border-border/60 pt-3">
