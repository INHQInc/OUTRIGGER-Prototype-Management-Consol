...
export async function POST(req: NextRequest) {
  let body: { key?: string; text?: string; answers?: string; refine?: { section?: string; correction?: string; current?: BriefDraft } };
  ...
  try {
    if (body.refine) {
      ...
      const draft = await refineBrief({ orgId: g.orgId, proto: g.proto, current, section: section as BriefSection, correction });
