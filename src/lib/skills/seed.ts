...
import { SYSTEM_SKILL, IDEAS_SKILL, PROTOTYPE_SKILL, BRIEF_AUTHOR_SKILL, INTEGRATION_PACKAGE_SKILL, EXPERIMENT_ANALYST_SKILL, MEASUREMENT_PLANNER_SKILL } from "./builtins";
...
async function seedBuiltins(): Promise<void> {
  ...
  for (const md of [PROTOTYPE_SKILL, SYSTEM_SKILL, IDEAS_SKILL, BRIEF_AUTHOR_SKILL, INTEGRATION_PACKAGE_SKILL, EXPERIMENT_ANALYST_SKILL, MEASUREMENT_PLANNER_SKILL]) {
    ...
    const CONSOLE_SKILLS = ["opmc-brief-author", "opmc-experiment-analyst", "opmc-measurement-planner"];
    await upsertSkill({ id, name: fm.name ?? id, scope: "global", description: fm.description ?? "", body: md, builtIn: true, delivery: CONSOLE_SKILLS.includes(id) ? "console" : "branch" });
  }
