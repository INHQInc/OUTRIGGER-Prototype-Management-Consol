...
*Updated: 2026-07-21 (post Sites-elimination + verification triage + workspace collapse). Read AGENTS.md first (model + rules), then this (state + next moves).*

## ⚠ UNPUSHED (handle first)

- Unpushed batch at handoff: `20a8543` (heartbeat self-verification), `e556c3a` (**Sites elimination**), docs ×2, `9477aae` (**workspace collapsed to three action cards**), then the verification-fix set: `fix(security)` fail-closed tenant guards + deleteOrg cascade + heartbeat id length + dead-links/counts + this doc. **User runs `git push`**; verify the deploy after via Vercel MCP.
- The 4-lens adversarial verification over `9e45c4e..HEAD` COMPLETED (31 agents): 27 confirmed findings, ALL triaged and fixed 2026-07-21. Headline class: three routes (versions/source/promotions) still resolved the org via the deleted Site chain → guards silently skipped for siteKey="" prototypes (two reachable unauthenticated via the middleware `Bearer opmc_` pass-through). Fix: shared `guardPrototypeAccess()` in `src/lib/prototypes/guard.ts` — **fail-closed** (unresolvable org = 403). Rule going forward: every prototype-scoped route uses this guard; never re-introduce `getSite(proto.siteKey)` for authz; never write a guard that skips when the org is falsy.
...
- **Prototype workspace** `/prototypes/[key]` with tabs (real routes): **Pipeline** = three action cards (9477aae): **Build** (SourcePanel: repo status / get-started block / Cut version, version history in a disclosure), **Review** (primary lower env `?opmc=` link, others in a disclosure), **Experiment** (one "Send to Optimizely" hero button → latest version → production env; full per-env matrix + history in a disclosure). Stage = compact `StageSelect` dropdown in the header (stepper deleted). **Details** (editable: targets/hypothesis/metrics/brief/owner), **Settings** (Code location + delete).
